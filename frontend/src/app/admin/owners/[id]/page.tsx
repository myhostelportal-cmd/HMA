        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">Owned Hostels</h2>
              <div className="px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl border border-indigo-100 text-xs font-black uppercase tracking-widest">
                {owner.hostels?.length || 0} Total Units
              </div>
            </div>
                        ...
                        <div className="p-6 flex flex-col h-full">
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <h3 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{hostel.hostel_name}</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hostel ID: #{hostel.hostel_id}</p>
                              </div>
                            </div>
                            <div className="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-all">
                              <ChevronRight size={20} />
                            </div>
                          </div>

                          <div className="space-y-4">
                            {/* Location + Total Capacity on the same row */}
                            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <p className="text-sm font-medium whitespace-nowrap">{hostel.total_capacity} Total Capacity</p>
                              </div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-sm font-bold text-slate-900 truncate">{hostel.total_students || 0} Students</p>
                              </div>
                              
                              {/* Management Section */}
                              <div className="space-y-1 text-right">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Warden Assigned</p>
                                <p className="text-sm font-bold text-slate-900 truncate">{hostel.warden_name || 'Not Assigned'}</p>
                                <p className="text-xs font-bold text-slate-400 mt-1">ID: #{hostel.warden_id || 'N/A'}</p>
                              </div>
                            </div>

                            {/* Revenue Section (Optional line) */}
                            <div className="pt-3 border-t border-slate-50 flex items-center justify-between">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Revenue</span>
                              <span className="text-sm font-black text-indigo-600">₹{(hostel.total_revenue || 0).toLocaleString()}</span>
                            </div>
                          </div>
                        </div>
                      </Card>
