                ...
                <form onSubmit={handleUpdateAdmin} className="space-y-6">
                  ...
                  <div className="pt-4 border-t border-slate-50">
                    <button 
                      ...
                      className="flex items-center gap-2 text-blue-600 hover:text-blue-700 font-bold text-sm transition-colors group disabled:opacity-50"
                    >
                      <div className="p-2 bg-blue-50 rounded-lg group-hover:bg-blue-100 transition-colors">
                        {resetting === selectedAdmin.admin_id ? <Loader2 size={16} className="animate-spin" /> : <Key size={16} />}
                      </div>
                      {resetting === selectedAdmin.admin_id ? 'Sending Link...' : 'Reset Admin Password'}
                    </button>
                  </div>
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  <button 
                    disabled={resetting !== null}
                    onClick={() => handleResetPassword(selectedAdmin.admin_id)}
                    className="w-full flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-700 text-white font-black py-5 rounded-2xl shadow-xl shadow-blue-600/20 transition-all active:scale-95 disabled:opacity-50"
                  >
                    {resetting === selectedAdmin.admin_id ? <Loader2 size={20} className="animate-spin" /> : <Key size={20} />}
                    {resetting === selectedAdmin.admin_id ? 'Sending Reset Link...' : 'Send Password Reset Link'}
                  </button>
