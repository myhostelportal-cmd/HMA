              ...
              <Link key={hostel.hostel_id} href={`/admin/hostels/${hostel.hostel_id}`}>
                <Card className="hover:scale-[1.02] transition-transform duration-300 cursor-pointer group relative">
                  ...
                  <div className="space-y-4">
                    {/* Location + Total Rooms on the same row */}
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-medium whitespace-nowrap">{hostel.total_rooms} Total Rooms</p>
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-bold text-slate-900 truncate">{hostel.owner_name || 'Not Assigned'}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Owner ID</p>
                        <p className="text-xs font-bold text-slate-600">{hostel.owner_id ? `#${hostel.owner_id}` : 'N/A'}</p>
                      </div>
                      
                      {/* Warden Section */}
                      <div className="space-y-1 text-right">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Warden Name</p>
                        <p className="text-sm font-bold text-slate-900 truncate">{hostel.warden_name || 'Not Assigned'}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Warden ID</p>
                        <p className="text-xs font-bold text-slate-600">{hostel.warden_id ? `#${hostel.warden_id}` : 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </Card>
