...
router.get('/stats', async (req, res) => {
  ...
  try {
    ...
    // --- Total Hostels Count ---
    const totalHostelsRes = await db.query(`SELECT COUNT(*) as count FROM hostels WHERE owner_id = $1`, [ownerId]);
    const totalHostels = parseInt(totalHostelsRes.rows[0]?.count) || 0;
